...
// Only what is actually need (instead of <lm.h>)
#include <lmuse.h>
#include <lmapibuf.h>
#include <lmerr.h>
...
namespace {
    ...
    // Wrapper for Win32 HANDLE values that use CloseHandle.
    class ScopedHandle {
    public:
    ...
    // Wrapper for WTS server handles that use WTSCloseServer.
    class ScopedWtsServerHandle {
    public:
        ScopedWtsServerHandle() = default;
        explicit ScopedWtsServerHandle(HANDLE handle) : handle_(handle) {}
        ...
        ScopedWtsServerHandle(ScopedWtsServerHandle&& other) noexcept : handle_(other.release()) {}
        ScopedWtsServerHandle& operator=(ScopedWtsServerHandle&& other) noexcept {
            if (this != &other) reset(other.release());
            return *this;
        }

        ~ScopedWtsServerHandle() { reset(); }
    ...
    // Reverts impersonation on scope exit.
    ...
    public:
        ...
        ~ScopedImpersonation() {
            ...
            // token_ will be closed by ScopedHandle destructor
        }

        // Begin takes ownership of an impersonation token (TokenImpersonation).
        bool Begin(HANDLE impersonationToken, std::wstring& errorMessage) {
            if (!impersonationToken || impersonationToken == INVALID_HANDLE_VALUE) {
                errorMessage = L"No impersonation token provided.";
                return false;
            }
            token_.reset(impersonationToken);
    ...
    bool PromptForPassword(const std::wstring& userName, std::wstring& password) {
        ...
        const bool hasConsoleMode = inputHandle != nullptr && inputHandle != INVALID_HANDLE_VALUE &&
            GetConsoleMode(inputHandle, &originalMode) != FALSE;
        if (hasConsoleMode) SetConsoleMode(inputHandle, originalMode & ~ENABLE_ECHO_INPUT);
        const bool readSucceeded = static_cast<bool>(std::getline(std::wcin, password));
        if (hasConsoleMode) SetConsoleMode(inputHandle, originalMode);
        std::wcout << L"\n";
        return readSucceeded;
    }

    // Create an initial primary token (LogonUserW) when /U supplied. Returns token in 'token' (owns it).
    ...
    bool WildcardMatchRecursive(const wchar_t* pattern, const wchar_t* text) {
        if (*pattern == L'\0') return *text == L'\0';
        if (*pattern == L'*') return WildcardMatchRecursive(pattern + 1, text) ||
            (*text != L'\0' && WildcardMatchRecursive(pattern, text + 1));
        if (*pattern == L'?') return *text != L'\0' && WildcardMatchRecursive(pattern + 1, text + 1);
    ...
    bool ParseArgs(int argc, wchar_t* argv[], Options& options) {
        for (int i = 1; i < argc; ++i) {
            ...
            if (argUpper == L"/?") { options.showHelp = true; continue; }
            if (argUpper == L"/F") { options.force = true; continue; }
            if (argUpper == L"/T") { options.tree = true; continue; }
            if (argUpper == L"/PID") {
                if (i + 1 >= argc) { std::wcout << L"ERROR: Missing value for /PID.\n"; return false; }
                DWORD pid = 0;
                if (!ParsePid(argv[++i], pid)) { std::wcout << L"ERROR: Invalid PID value: " << argv[i] << L"\n"; return false; }
                options.pids.push_back(pid);
                continue;
            }
            if (argUpper == L"/IM") {
                if (i + 1 >= argc) { std::wcout << L"ERROR: Missing value for /IM.\n"; return false; }
                options.imageNames.emplace_back(argv[++i]);
                continue;
            }
            if (argUpper == L"/S") {
                if (i + 1 >= argc) { std::wcout << L"ERROR: Missing value for /S.\n"; return false; }
                options.remoteSystem = argv[++i];
                continue;
            }
            if (argUpper == L"/U") {
                if (i + 1 >= argc) { std::wcout << L"ERROR: Missing value for /U.\n"; return false; }
                options.user = argv[++i];
                continue;
            }
            if (argUpper == L"/P") {
                if (i + 1 < argc && argv[i + 1][0] != L'/') { options.password = argv[++i]; options.promptForPassword = false; }
                else { options.password.clear(); options.promptForPassword = true; }
                continue;
            }
        ...
        if (!options.remoteSystem.empty() && options.promptForPassword && options.user.empty()) {
            std::wcout << L"ERROR: /P requires /U to be specified.\n";
            return false;
        }
        if (!options.remoteSystem.empty() && !options.password.empty() && options.user.empty()) {
            std::wcout << L"ERROR: /P requires /U to be specified.\n";
            return false;
        }
        if (!options.remoteSystem.empty() && options.remoteSystem == L".") options.remoteSystem.clear();
        return true;
    }
    ...
    bool ReadFormattedCounterArray(HCOUNTER counter, std::vector<CounterValue>& values) {
        ...
        values.clear(); values.reserve(itemCount);
        for (DWORD i = 0; i < itemCount; ++i) {
            if (items[i].FmtValue.CStatus != ERROR_SUCCESS) continue;
            CounterValue v;
            v.instanceName = items[i].szName ? items[i].szName : L"";
            v.value = items[i].FmtValue.largeValue;
    ...
    bool PopulateRemoteParentPids(const std::wstring& remoteSystem, std::vector<ProcessInfo>& processes, std::wstring& errorMessage) {
        ...
        if (PdhOpenQueryW(nullptr, 0, &query) != ERROR_SUCCESS) { errorMessage = L"PdhOpenQueryW failed."; return false; }

        HCOUNTER pidCounter = nullptr; HCOUNTER parentCounter = nullptr;
        ...
        if (collectStatus != ERROR_SUCCESS) { PdhCloseQuery(query); errorMessage = L"PdhCollectQueryData failed."; return false; }
        ...
        if (!pidRead || !parentRead) { errorMessage = L"Could not read remote process performance counter data."; return false; }

        std::map<std::wstring, std::vector<size_t>> parentIndexesByName;
        for (size_t i = 0; i < parentValues.size(); ++i) parentIndexesByName[parentValues[i].instanceName].push_back(i);
        ...
        for (ProcessInfo& process : processes) {
            ...
            if (it != parentsByPid.end()) process.parentPid = it->second;
        }
        return true;
    }
    ...
    bool EnumerateRemoteProcesses(HANDLE serverHandle, const std::wstring& remoteSystem, bool includeParentPids, std::vector<ProcessInfo>& processes, std::wstring& errorMessage) {
        processes.clear();
        DWORD level = 1; DWORD count = 0; PWTS_PROCESS_INFO_EXW processInfo = nullptr;
        ...
        for (DWORD i = 0; i < count; ++i) {
            ProcessInfo p;
            p.pid = processInfo[i].ProcessId;
            p.imageName = processInfo[i].pProcessName ? processInfo[i].pProcessName : L"";
            processes.push_back(std::move(p));
        }
        WTSFreeMemoryExW(WTSTypeProcessInfoLevel1, processInfo, count);
        if (includeParentPids && !PopulateRemoteParentPids(remoteSystem, processes, errorMessage)) return false;
        return true;
    }

    std::vector<DWORD> BuildTreeOrder(DWORD rootPid, const std::map<DWORD, std::vector<DWORD>>& childrenByParent, std::set<DWORD>& visited) {
        std::vector<DWORD> order;
        if (!visited.insert(rootPid).second) return order;
    ...
    std::vector<DWORD> ResolveTargetPids(const Options& options, const std::vector<ProcessInfo>& processes) {
        ...
        for (const std::wstring& imagePattern : options.imageNames) {
            for (const ProcessInfo& process : processes) {
                if (WildcardMatch(imagePattern, process.imageName)) uniqueTargets.insert(process.pid);
            }
        }
    ...
    const ProcessInfo* FindProcess(const std::vector<ProcessInfo>& processes, DWORD pid) {
        for (const ProcessInfo& p : processes) if (p.pid == pid) return &p;
        return nullptr;
    }

    int RunTaskkill(const Options& options) {
        const bool isRemote = !options.remoteSystem.empty();
        ScopedHandle remoteToken; // owns primary token from LogonUserW
        ScopedImpersonation impersonation; // owns impersonation token while active
        ...
        if (isRemote) {
            ...
            // If user supplied credentials, remoteToken contains a primary token from LogonUserW.
            // Duplicate it to get an impersonation token suitable for ImpersonateLoggedOnUser.
            if (remoteToken.get() && remoteToken.get() != INVALID_HANDLE_VALUE) {
                HANDLE impToken = INVALID_HANDLE_VALUE;
                if (!DuplicateTokenEx(remoteToken.get(), MAXIMUM_ALLOWED, nullptr,
                    SecurityImpersonation, TokenImpersonation, &impToken)) {
                    std::wcout << L"ERROR: DuplicateTokenEx failed: " << FormatSystemMessage(GetLastError()) << L"\n";
                    return 1;
                }

                if (!impersonation.Begin(impToken, errorMessage)) {
                    std::wcout << L"ERROR: " << errorMessage << L"\n";
                    CloseHandle(impToken);
                    return 1;
                }
                // Now we are impersonating the supplied user context.
            }

// Before opening WTS server, create an authenticated IPC$ connection using NetUseAdd.

// Build IPC path
std::wstring ipc = L"\\\\";
ipc += options.remoteSystem;
ipc += L"\\IPC$";

NET_API_STATUS netStatus = NERR_Success;

USE_INFO_2 ui{};
ZeroMemory(&ui, sizeof(ui));  // extra safety

ui.ui2_remote = const_cast<LPWSTR>(ipc.c_str());

// REQUIRED for NetUseAdd level 2
ui.ui2_asg_type = USE_IPC | USE_DISKDEV;  // safer / more compatible

// Credentials
if (!options.user.empty()) {
    ui.ui2_username = const_cast<LPWSTR>(options.user.c_str());
    ui.ui2_password = const_cast<LPWSTR>(options.password.c_str());
} else {
    ui.ui2_username = nullptr;
    ui.ui2_password = nullptr;
}

// Create network connection
netStatus = NetUseAdd(nullptr, 2, reinterpret_cast<LPBYTE>(&ui), nullptr);


            // Open WTS server (use leading backslashes); prefer WTSOpenServerEx if available.
            std::wstring serverName = L"\\\\";
            serverName += options.remoteSystem;
            std::vector<wchar_t> buf(serverName.c_str(), serverName.c_str() + serverName.size() + 1);
            serverHandle.reset(WTSOpenServerW(buf.data()));
            if (!serverHandle.get() || serverHandle.get() == INVALID_HANDLE_VALUE) {
                std::wcout << L"ERROR: Could not connect to remote system \"" << options.remoteSystem
                    << L"\": " << FormatSystemMessage(GetLastError()) << L"\n";
                // Clean up the NetUse session we created
                NetUseDel(nullptr, const_cast<LPWSTR>(ipc.c_str()), 0);
                return 1;
            }
            ...
            if (!EnumerateRemoteProcesses(serverHandle.get(), options.remoteSystem, options.tree, processes, errorMessage)) {
                std::wcout << L"ERROR: Could not enumerate processes on remote system \"" << options.remoteSystem
                    << L"\": " << errorMessage << L"\n";
                WTSCloseServer(serverHandle.get());
                serverHandle.reset();
                NetUseDel(nullptr, const_cast<LPWSTR>(ipc.c_str()), 0);
                return 1;
            }

            // Cleanup NetUse (remove IPC$ mapping) after enumeration is done.
            NetUseDel(nullptr, const_cast<LPWSTR>(ipc.c_str()), 0);

            // Impersonation will be reverted when 'impersonation' goes out of scope.
        }
        else {
        ...
        for (const ProcessInfo& process : processes) childrenByParent[process.parentPid].push_back(process.pid);
        ...
        for (const DWORD target : targets) {
            ...
            for (const DWORD pid : order) {
                ...
                if (options.force) {
                    std::wcout << L"SUCCESS: The process \"" << imageName << L"\" with PID " << pid << L" has been terminated.\n";
                    continue;
                }
